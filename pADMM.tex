...
\icmltitlerunning{Probabilistic Methods for Large-Scale Distributed Learning}

\begin{document}

\twocolumn[
\icmltitle{Probabilistic Methods for Large-Scale Distributed Learning}
...
% This type of coordinate-wise algorithm is efficient and scalable to large datasets.
% their non-probabilistic counterpart in optimization literature, coordinate descent algorithms,  regains much interest in statistics and ML community since \cite{Friedman2008, Banerjee2008}, and are now successfully applied in various applications \cite{Friedman2010, Bradley2011, Yuan2012, Yu2013}.
...
\end{equation}
Note that for the gradient-ascent update in (\ref{admm_update_lag}) the step size is chosen to be $\rho$. The motivation behind this choice is explained in \yrcite{Boyd10}, that by using $\rho$ as the step size, the iteration
...
%At last, ADMM and the proposed HM, ADMM methods lies in the middle of the above two frameworks, which require moderately less number of iterations than distributed sub-gradient methods and may overcome some difficulties of AVGM in certain scenarios.
